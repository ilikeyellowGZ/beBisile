...
export const errorMiddleware: ErrorRequestHandler = (error, _req, res, _next) => {
  ...
  const databaseUnavailable = details.name === 'MongooseServerSelectionError' || details.name === 'MongoServerSelectionError' || details.name === 'MongoNetworkError';
  const status = Number(details.statusCode || details.status || (details.code === 11000 ? 409 : databaseUnavailable ? 503 : 500));
