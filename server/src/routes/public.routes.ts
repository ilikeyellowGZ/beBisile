...
publicRoutes.post('/contact', asyncHandler(async (req, res) => {
...
publicRoutes.post('/newsletter/subscribe', asyncHandler(async (req, res) => {
...
publicRoutes.post('/reviews', asyncHandler(async (req, res) => res.status(201).json({ review: await Review.create({ ...req.body, status: 'pending' }) })));
publicRoutes.get('/products/:productId/reviews', asyncHandler(async (req, res) => res.json({ reviews: await Review.find({ productId: req.params.productId, status: 'approved' }).lean() })));
