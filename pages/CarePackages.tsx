  ...
  <div className="min-h-screen bg-secondary pt-20">
    ...
    <section className="mx-auto max-w-[1480px] px-6 py-16 md:px-12 md:py-20">
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        {packages.map((item, index) => <FadeIn key={item.name} delay={index * 80} className="bisile-card-surface flex h-full flex-col"><div className="aspect-[4/3] overflow-hidden"><OptimizedImage src={item.image} width={900} widths={[360, 540, 720, 900, 1200]} sizes="(min-width: 1024px) 33vw, 100vw" alt={item.name} className="editorial-image h-full w-full object-cover" /></div><div className="flex flex-1 flex-col p-7"><p className="text-[9px] uppercase tracking-[0.18em] text-accent">{item.subtitle}</p><h3 className="mt-3 font-subhead text-3xl">{item.name}</h3><p className="mt-2 text-xs text-[#5B3A24]/62">{item.price}</p><ul className="mt-6 flex-1 space-y-3 border-t border-[#B9AA8B]/36 pt-5 text-xs leading-6 text-[#5B3A24]/66">{item.includes.map((line) => <li key={line} className="flex gap-3"><Check size={13} className="mt-1 shrink-0 text-accent" />{line}</li>)}</ul><a href={getWhatsAppUrl(`Hello BISILE, I would like to order ${item.name}.`)} target="_blank" rel="noreferrer" className="mt-7 flex items-center justify-between border border-[#8A6F35]/60 px-5 py-4 text-[10px] uppercase tracking-[0.18em] text-[#5B3A24] hover:border-accent hover:text-accent">Select package <ArrowRight size={14} /></a></div></FadeIn>)}
      </div>
    </section>
  </div>
);
