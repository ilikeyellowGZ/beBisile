...
const discoverItems: DiscoverItem[] = [
  ...
  },
  {
    ...
    image: packageImages.carePouches,
  },
];
...
const topCenterPreviewLabels = new Set(['Fragrances', 'Premium Hair', 'Diffuser']);
        ...
        {previews.map((item) => (
          <Link key={`${item.label}-${activeIndex}`} className="group animate-fade-in block" to={item.path} data-discover="true">
            <div className="bisile-image-frame aspect-[4/3]">
              <OptimizedImage alt={item.label} className={`editorial-image h-full w-full object-cover ${topCenterPreviewLabels.has(item.label) ? '!object-top' : ''}`} src={item.image} width={900} widths={[360, 540, 720, 900, 1200]} sizes="(min-width: 768px) 50vw, 100vw" />
            </div>
