...
export const Navbar: React.FC = () => {
  ...
  return (
    <>
      <nav onMouseLeave={() => setActiveMenu(null)} className="fixed top-0 z-50 w-full bg-white/95 text-primary backdrop-blur">
        <div className="bisile-shell relative flex h-16 items-center justify-between">
          ...
          <Link to="/" className="absolute left-1/2 z-20 flex -translate-x-1/2 items-center justify-center transition-opacity hover:opacity-75">
            <img src={brandImages.logo} alt="BISILE - Be Luxury" className="h-9 w-auto object-contain md:h-10" />
          </Link>
          ...
          <div className="hidden items-center justify-end gap-7 md:flex">
            ...
            <div className="ml-1 flex items-center gap-5 border-l border-[#e5e2dd] pl-6">
              <button onClick={openSearch} className="text-primary/80 transition-colors hover:text-accent" aria-label="Search">
          ...
          <div className="ml-auto flex items-center justify-end gap-4 md:hidden">
            <button onClick={openSearch} className="text-primary" aria-label="Search">
      ...
      <div className={`fixed inset-0 z-[70] overflow-y-auto bg-[#fbfaf8] text-primary transition-all duration-500 ease-out ${isSearchOpen ? 'pointer-events-auto opacity-100' : 'pointer-events-none opacity-0'}`} role="dialog" aria-modal="true" aria-label="Search BISILE">
        <div className={`min-h-full transition-transform duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] ${isSearchOpen ? 'translate-y-0' : '-translate-y-5'}`}>
          <div className="bisile-shell relative flex h-16 items-center justify-end border-b border-[#e5e2dd]">
            ...
            <button onClick={() => setIsSearchOpen(false)} className="flex h-11 w-11 items-center justify-center text-primary/65 transition-colors hover:text-accent" aria-label="Close search">
          ...
          <div className="bisile-shell pb-12 pt-11 md:pb-20 md:pt-16">
            <div className="mx-auto max-w-[980px]">
              ...
              <div className="mt-8 border-b border-primary/20 pb-4 md:mt-10">
                <label className="sr-only" htmlFor="bisile-search">Search BISILE</label>
                <div className="flex items-center gap-3 md:gap-5">
                  <Search size={21} strokeWidth={1.15} className="shrink-0 text-primary/42" />
                  <input
                    id="bisile-search"
                    ref={searchInputRef}
                    value={searchQuery}
                    ...
                    className="w-full bg-transparent font-inter text-2xl font-light leading-tight text-primary outline-none placeholder:text-primary/28 sm:text-3xl md:text-4xl"
                  />
                </div>
              </div>
              ...
              <div className="mt-5 divide-y divide-[#e5e2dd] border-y border-[#e5e2dd]">
                {searchResults.map((item, index) => (
                  <Link
                    ...
                    className={`group grid grid-cols-[64px_minmax(0,1fr)_auto] items-center gap-4 py-4 transition-all duration-300 hover:bg-white sm:grid-cols-[80px_minmax(0,1fr)_auto] sm:gap-6 sm:px-4 ${isSearchOpen ? 'translate-y-0 opacity-100' : 'translate-y-3 opacity-0'}`}
                  >
                    <div className="flex h-20 w-16 shrink-0 items-center justify-center overflow-hidden bg-white sm:h-24 sm:w-20">
              ...
              {searchResults.length === 0 && (
                <div className="mt-6 border-y border-[#e5e2dd] py-8 font-inter text-sm font-light text-primary/58">
      ...
      <div className={`fixed inset-0 z-40 overflow-y-auto bg-[#fbfaf8] px-6 pb-10 pt-24 transition-transform duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] md:hidden ${isMobileMenuOpen ? 'translate-y-0' : '-translate-y-full'}`}>
        <div className={`transition-all duration-500 ease-out ${isMobileMenuOpen ? 'translate-y-0 opacity-100' : '-translate-y-3 opacity-0'}`}>
          ...
          <div className="border-y border-[#e5e2dd]">
            {[...leftLinks, ...rightLinks].map((item, index) => (
              <button
                key={item.label}
                onClick={() => goTo(item)}
                className="group grid w-full grid-cols-[2.5rem_minmax(0,1fr)_auto] items-center gap-3 border-b border-[#e5e2dd] py-5 text-left last:border-b-0"
              >
