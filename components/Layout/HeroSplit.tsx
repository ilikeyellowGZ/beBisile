...
const HeroPanelView: React.FC<{
  ...
  const mediaFitClass = panel.id === 'fragrance'
    ? 'object-contain object-center bg-[#f7f5f1] p-8 sm:p-10 lg:p-12'
  ...
  return (
    <Link
      ...
      onBlur={onReset}
      className={`group relative flex min-h-[48svh] overflow-hidden bg-black text-white transition-[flex-basis,filter] duration-700 ease-[cubic-bezier(0.22,1,0.36,1)] lg:min-h-0 lg:basis-1/2 ${
    ...
    >
      {canRenderVideo ? (
        <video
          ...
          preload="metadata"
          poster={panel.poster}
          className={`absolute inset-0 h-full w-full ${mediaFitClass} transition-[filter,transform,opacity] duration-700 ${
            isActive ? 'scale-100 opacity-100 contrast-105' : 'scale-100 opacity-95 contrast-95'
          } ${isInactive ? 'opacity-80 contrast-90' : ''}`}
        >
      ...
      <div className={`absolute inset-0 transition-colors duration-700 ${isActive ? 'bg-black/20' : isInactive ? 'bg-black/68' : 'bg-black/40'}`} />
...
export const HeroSplit: React.FC = () => {
  ...
  return (
    <section className="relative h-[calc(100svh-4rem)] min-h-[720px] w-full overflow-hidden bg-black lg:h-[calc(100vh-4rem)]" aria-label="BISILE hero">
